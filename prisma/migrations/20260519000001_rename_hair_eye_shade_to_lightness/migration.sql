-- ─── Rebuild mv_person_current_state with renamed hair/eye lightness cols ──
-- Hair/eye shade is now interpreted as absolute Lightness (matches the
-- professional Level system from L'Oréal/Wella/etc). The catalog data itself
-- was re-classified in color-catalog.ts; this migration renames the MV columns
-- and indexes so the code reads self-documenting.
...
SELECT
  ...
  -- Hair (folded current persona hair color → catalog lookup, ABSOLUTE)
  ...
  -- Eye (Person.eyeColor → catalog lookup, ABSOLUTE)
  ...
  -- Skin (folded extensible attribute "skin_tone" → catalog lookup).
  -- Skin's secondary axis stays Undertone (Cool/Warm/Neutral) — orthogonal,
  -- not a lightness scale, so its meaning doesn't change in this rename.
