-- ─── Rebuild mv_person_current_state with hue/shade columns ─────────────────
-- Drop the v1 MV (already dropped by the color_catalog migration, but be safe)
...
SELECT
  ...
  -- Hair (folded current persona hair color → catalog lookup)
  lookup_hair_hue(ft."currentHairColor")        AS "hairHue",
  lookup_hair_shade(ft."currentHairColor")      AS "hairShade",
  lookup_hair_shade_rank(ft."currentHairColor") AS "hairShadeRank",

  -- Eye (Person.eyeColor → catalog lookup)
  lookup_eye_hue(p."eyeColor")        AS "eyeHue",
  lookup_eye_shade(p."eyeColor")      AS "eyeShade",
  lookup_eye_shade_rank(p."eyeColor") AS "eyeShadeRank",

  -- Skin (folded extensible attribute "skin_tone" → catalog lookup). Stays NULL
  -- until admin adds a "skin_tone" attribute in Settings + populates per-person.
...
CREATE INDEX mv_pcs_hair_shade_rank  ON mv_person_current_state ("hairShadeRank");
CREATE INDEX mv_pcs_eye_hue          ON mv_person_current_state ("eyeHue");
CREATE INDEX mv_pcs_eye_shade_rank   ON mv_person_current_state ("eyeShadeRank");
