-- Rebuild mv_person_current_state with secondary-color lookups (heterochromia
-- eye secondary, hair highlights/ombre/two-tone secondary). Both flow through
-- the existing color_catalog lookup functions (same as primary colors), so
-- they get hue + lightness classification for free.
...
folded AS (
  SELECT
    ...
    max(ordered."currentSecondaryHairColor") FILTER (WHERE ordered."currentSecondaryHairColor" IS NOT NULL) OVER w AS "currentSecondaryHairColor",
...
folded_top AS (
  SELECT "personId", "currentHairColor", "currentSecondaryHairColor", "currentWeight", "currentBuild"
  FROM folded
  WHERE rn = max_rn
),
attr_ordered AS (
...
SELECT
  ...
  -- Secondary hair (highlights / balayage / ombre / two-tone)
  lookup_hair_hue(ft."currentSecondaryHairColor")        AS "secondaryHairHue",
  lookup_hair_shade(ft."currentSecondaryHairColor")      AS "secondaryHairLightness",
  lookup_hair_shade_rank(ft."currentSecondaryHairColor") AS "secondaryHairLightnessRank",
  ...
  -- Secondary eye (complete heterochromia — the other iris)
  lookup_eye_hue(p."secondaryEyeColor")        AS "secondaryEyeHue",
  lookup_eye_shade(p."secondaryEyeColor")      AS "secondaryEyeLightness",
  lookup_eye_shade_rank(p."secondaryEyeColor") AS "secondaryEyeLightnessRank",

  -- Skin (unchanged from MV v2)
...
CREATE INDEX mv_pcs_sec_hair_hue           ON mv_person_current_state ("secondaryHairHue");
CREATE INDEX mv_pcs_sec_hair_lightness_rank ON mv_person_current_state ("secondaryHairLightnessRank");
...
CREATE INDEX mv_pcs_sec_eye_hue            ON mv_person_current_state ("secondaryEyeHue");
CREATE INDEX mv_pcs_sec_eye_lightness_rank ON mv_person_current_state ("secondaryEyeLightnessRank");
