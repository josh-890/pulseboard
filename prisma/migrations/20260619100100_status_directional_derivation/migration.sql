...
--
-- 1. Backfill: every existing surgical scalar delta is a breast augmentation
--    (the only surgical scalar case in the data), so re-tag SURGICAL -> AUGMENTATION.
--    Body-event SURGICAL (BodyMarkEvent/BodyModificationEvent) is left untouched.
-- 2. Reissue app_recompute_person_current_state with the directional status fold:
--      AUGMENTATION | SURGICAL (legacy) -> ENHANCED
--      REDUCTION                        -> REDUCED
--      REVERSAL                         -> RESTORED
--      surgical kind in history overridden by a natural delta -> RESTORED
--      else                             -> NATURAL
--    Kept in lockstep with deriveStatus() in src/lib/services/person-service.ts.
-- 3. Recompute every PersonCurrentState so attributeStatuses reflects the new fold.

UPDATE "ScalarDelta" SET cause = 'AUGMENTATION' WHERE cause = 'SURGICAL';
...
AS $function$
  ...
  -- ADR-0018: any surgical-kind delta counts toward "intervention in history".
  intervention_history AS (
    SELECT DISTINCT "personId", slug
    FROM delta_ranked
    WHERE cause IN ('SURGICAL', 'AUGMENTATION', 'REDUCTION', 'REVERSAL')
  ),
  status_per_attr AS (
    SELECT
      dl."personId",
      dl.slug,
      CASE
        WHEN dl.cause IN ('AUGMENTATION', 'SURGICAL') THEN 'ENHANCED'
        WHEN dl.cause = 'REDUCTION' THEN 'REDUCED'
        WHEN dl.cause = 'REVERSAL'  THEN 'RESTORED'
        WHEN EXISTS (SELECT 1 FROM intervention_history sh
...
-- Refresh every PersonCurrentState so attributeStatuses reflects the new fold.
