-- ─── Color family lookup (driven from src/lib/constants/color-families.ts) ──
CREATE TABLE color_family_map (
...
-- Schema-qualify table + unaccent so SQL function inlining at MV-creation time
-- doesn't lose the search_path. Explicit ''::text cast avoids unknown-type
-- ambiguity for unaccent(text|regdictionary, text) overload resolution.
CREATE OR REPLACE FUNCTION compute_hair_family(v TEXT) RETURNS TEXT AS $$
  SELECT family FROM public.color_family_map
  WHERE category = 'hair'
    AND value_norm = public.unaccent(lower(trim(coalesce(v, ''::text))))
  LIMIT 1;
$$ LANGUAGE SQL STABLE;

CREATE OR REPLACE FUNCTION compute_eye_family(v TEXT) RETURNS TEXT AS $$
  SELECT family FROM public.color_family_map
  WHERE category = 'eye'
    AND value_norm = public.unaccent(lower(trim(coalesce(v, ''::text))))
  LIMIT 1;
...
CREATE OR REPLACE FUNCTION compute_skin_family(v TEXT) RETURNS TEXT AS $$
  SELECT family FROM public.color_family_map
  WHERE category = 'skin'
    AND value_norm = public.unaccent(lower(trim(coalesce(v, ''::text))))
  LIMIT 1;
...
-- ─── Rebuild mv_person_current_state with extended search columns ───────────
...
SELECT
  ...
  ft."currentWeight",
  ft."currentBuild",
  compute_hair_family(ft."currentHairColor") AS "hairColorFamily",
  compute_eye_family(p."eyeColor")           AS "eyeColorFamily",
...
CREATE INDEX mv_pcs_hair_family      ON mv_person_current_state ("hairColorFamily");
CREATE INDEX mv_pcs_eye_family       ON mv_person_current_state ("eyeColorFamily");
