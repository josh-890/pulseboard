-- Back out the hair secondary-color infrastructure. Highlights/balayage are
-- captured by the Hair Pattern attribute; the dedicated structured-search
-- column was overkill (nobody searches by highlight color in practice).
-- Eye secondary stays — heterochromia genuinely needs it.

-- Drop the MV first (it references the column we're about to drop)
DROP MATERIALIZED VIEW IF EXISTS mv_person_current_state;

-- Drop the column
ALTER TABLE "PersonaPhysical" DROP COLUMN IF EXISTS "currentSecondaryHairColor";

-- Recreate the MV without the secondary-hair columns / fold / lookups
...
SELECT
  ...
  -- Secondary eye (complete heterochromia — the other iris). Stays because
  -- heterochromia is a genuinely distinct color condition.
