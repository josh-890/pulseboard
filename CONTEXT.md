...
**Folder attribution** (`_cast.txt` in an archive folder):
...
**People files** (`_pulseboard_cast.txt` app-written, `_cast.txt` hand-written; added 2026-08-08, ADR-0029):
The archive's answer to *who is in this set* when nothing is running. The **generated** file mirrors what the app knows, in two labelled sections — `# credited` (the linked set's cast) and `# claimed` (the folder's attributions) — refreshed by the agent whenever its `# revision:` header differs from the server's. The **hand-written** file is your own assertion, read-only to the app, and only ever *adds*: removing a line takes nothing back. Both use one line format, `Name (ICG-ID)`, so one parser and one grep serve both. Deliberately plain text: `ConvertTo-Json` collapses a one-element array, and a participant list is exactly that shape.
A name may be written with underscores or spaces and in any capitalisation — `Iveta_C_(IC-87VY)`, `Iveta C (IC-87VY)` and `iveta c (IC-87VY)` are one person, because the **ICG-ID is the identity** and the name is provenance. Only the separators are normalised on read; the letters stay as written, since title-casing someone's name is a guess.
_Avoid_: calling either one a backup (`db-backup.sh` is); putting people into `_pulseboard.json` (that file is the identity anchor, and every rewrite risks the `archiveKey`); trusting `_pulseboard_index.tsv` over the per-folder files — it is generated from them.
