...
**`attribution-confirm-service.ts`** — Group confirmation (ADR-0027, plan slice 5). `confirmAttributionGroup` writes `ArchiveFolderAttribution` rows and mints ghost `Contact`s (ADR-0022) — **it creates no Set, StagingSet or Person**. That split is measured, not stylistic: only 1.7% of the 28,377 suggestions point at a Person the app already has and 84.8% of suggested identities are unknown entirely, so bulk confirmation is identity *registration*; materialisation stays the curated per-person import path, which now arrives pre-informed about which folders are whose. `planAttributions` is **pure and the safety rule of the whole feature**: a folder is attributed to exactly the confirmed people *it itself suggested*, never to the group's verdict — in `MPL | nata` 80 folders name Nata and 3 name someone else, and those 3 are counted as dissent, not swept along. Also `decideAttributionGroup` (`NOT_A_PERSON` / `SKIPPED` — the largest group in the archive is `W4B | w4b magazine`, 204 folders of a magazine title), `undoAttributionGroup` (drops verdict + its attributions; keeps the deduplicated contacts), `getAttributionQueue` (leverage-ordered, decided groups filtered out — a decision must survive the agent's next run), `getGroupFolders` (resolves `coverUrl` server-side; `buildUrl` reads the tenant bucket via AsyncLocalStorage).
...
| `attribution-actions.ts` | `confirmAttributionGroupAction` (one group per call — **no "confirm all visible"**; a single confirmation touches up to 204 folders and until the real error rate is known the rate limiter is the operator's hand), `markGroupNotAPersonAction`, `skipGroupAction`, `undoAttributionGroupAction`, `getGroupFoldersAction` (loaded on expand, not with the list) |
