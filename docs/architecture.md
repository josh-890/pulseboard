...
| `/collections/[id]` | `getCollectionWithItems()`, `getCollectionGalleryItems()` | `CollectionDetailGallery` — `MediaCollection.layout` (`GRID` \| `SIDE_BY_SIDE`, ADR-0014/D8). SIDE_BY_SIDE = comparison composite: a row of equal letterboxed cells (2…N) + per-cell reorder (`reorderCollection`), and an `ImageCompareSlider` (clip-path reveal) for pairs. Add-to-collection works from any lightbox via `GalleryLightbox enableCollections` (self-fetches `/api/collections/list`); Browse & Add multi-selects |
...
**`motif-template-service.ts`** — Alignment Templates (`MotifTemplate`): output aspect + `bakeLongSide` + target keypoints (frame fractions). A template binds to **either** a profile `slot` (legacy headshots, `slot` nullable+`@unique`) **or** a locus `MediaCategory` (`MediaCategory.alignmentTemplateId`, 0:1, ADR-0014) — slot XOR category. CRUD (create/update manage the category binding transactionally) + `getMotifTemplateForSlot` + `getMotifTemplateForCategory`. The **Motif Aligner** (`components/people/motif-aligner.tsx`) loads a source master, the user clicks the template's keypoints, and `lib/image/similarity-transform.ts` (pure, unit-tested 2D Umeyama fit) maps them onto the targets; the baked image is uploaded and tagged an **Aligned image** (`MediaItem.motifTemplateId` + `motifProvenance`, ADR-0013). **Slot mode** → `assignMotifImageAction` (HEADSHOT slot, `isAnnotation=true`); **category mode** → `assignAlignedImageAction` (DETAIL link to the category, NOT an annotation). Aligned headshots display via an aspect-preserving variant in `headshotDataFromLink`. Authored at `/settings/catalogs/motif-templates` (a "Bind to: Profile slot | Locus category" toggle); launched from the reference-session **Slot Manager** (slots) or a person's **Details tab** "Align" button (categories, via `getAlignmentTemplateForCategoryAction`) → `CrossSessionPicker`. The definition editor (`components/settings/motif-templates-catalog.tsx`) supports a **translucent reference-image underlay** for placing keypoints (drag-pan / wheel-zoom / rotate / opacity; guide only). Transient by default; **Pin** stores a raw downscaled webp via `POST /api/motif-templates/silhouette` (no MediaItem) and persists `MotifTemplate.silhouetteRef` + `silhouetteTransform` (JSONB).
...
8b. **Alignment templates & Aligned images** (ADR-0013/0014) — `MotifTemplate` (aspect, `bakeLongSide`, target keypoints) binds to **either** a profile `slot` **or** a locus `MediaCategory` (`MediaCategory.alignmentTemplateId`, 0:1; slot is nullable `@unique`). A baked **Aligned image** carries `MediaItem.motifTemplateId` + `motifProvenance` ({sourceMediaItemId, points, matrix}) — its identity, **never** `isAnnotation` (annotations are a separate crop/highlight concept). Aligned images are a derived copy in the reference session that retains provenance (re-bakeable). **Visibility:** excluded from the raw reference gallery and cross-session source pickers (`getPersonMediaGallery` / `getPersonMediaAcrossSessions` / `getPersonSessionsWithMedia` filter `motifTemplateId: null`), shown in their category (Details tab, with an "aligned" badge) and the cross-person Atlas (`/atlas`). Aligned headshots display via an aspect-preserving variant (not the 4:5 `profile_*` crop). The 5 profile slots fold into a "Profile" category group at the (pending) unification slice.

8c. **Slot management is consolidated** — the **Slot Manager** (`components/people/slot-manager.tsx`) on the **reference session** page is the single home for filling slots: per slot, **Standardize** (aligner) / **Link** (raw) / **Clear** / **★ default** (avatar = `isAvatar`). Uploads no longer auto-assign slots (`BatchUploadZone.computeMetadata` defaults to PORTFOLIO). The metadata-panel slot toggles are hidden on the ref session (`slotLabels={[]}`). The person-detail Reference Media Card is a read-only preview linking here. (The person Photos-tab lightbox still has a raw slot-assign — a secondary path left for a follow-up.)
