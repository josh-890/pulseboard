...
| `/people` (Nationality filter: `?nationality=IOC,IOC` — multi-select facet from `getPersonFacetCounts().nationality`, grouped on `Person.nationality`, so only nations somebody carries appear and the counts already respect the other filters. Flag + country name via `FlagImage`/`findCountryByCode`; the dropdown's A–Z ⇄ count switch is client state, not URL — `MultiFacetFilter.sortable`) | `getPersonsPaginated()`, `getHeadshotsForPersons()`, `getDistinct*()`, `countActiveContacts()` | `PersonList`, `BrowserToolbar`, `AddPersonSheet`, header **Contacts** link (count badge). `StatusFilter` carries a `watching=true` toggle (orthogonal to `status`) → `PersonFilters.watching`. An **ICG-ID** pill (`?idOrigin=external\|self`) → `PersonFilters.idOrigin`, a string predicate on the reserved marker (ADR-0026) with facet counts from `getPersonFacetCounts` |
...
**`alias-service.ts`** — Alias CRUD, channel linking, bulk import, merge. `getPersonAliases()` returns `creditCount` (combined `SetCreditRaw` + `SessionContribution` usages via `resolvedAliasId`) plus per-channel `setCount` corroboration. `createAlias()` also accepts `channelIds` to link at creation time and triggers participant-status refresh. **ADR-0024** (aliases in sets): `updateAlias()` guards an in-use rename (returns `needsRenameConfirm`) with `branchAliasToNew()` as the preferred resolution; the promotion queue is a derived query — `buildAliasPromotionCandidates()` (pure, unit-tested in `alias-promotion-queue.test.ts`) filters `SetCreditRaw` used-names not yet registered on the set's channel, `getAliasPromotionQueue()` wraps it, `promoteAliasFromQueue()` creates the alias + channel link + back-fills pins, `dismissAliasPromotion()` writes an `AliasPromotionDismissal`. Display precedence lives in the pure helper `lib/sets/credited-as.ts` (`resolveCreditedAs`: pinned alias → raw string → nothing), shared by the set hero, credits panels, and the career list. Backfill: `archive-service.ts scanArchiveForAliases()` captures used-names from confirmed single-participant folders.
...
**`parser.ts`** — Pure function: raw file text → `ParsedImportData` (person profile, digital identities, channel appearances, sets with co-model references, co-model directory). Handles edge cases (PowerShell artifacts, em-dash nulls, duplicate detection). One forward pass with one cursor, so the header must hand that cursor to the next section: it ends at the `=== Links aus 'Other Links' ===` marker **or** at the first `Channel :` / `Titeltxt :` line, because Other Links is optional and a file without it used to run the header loop to EOF (Hilary C, AX-00ET: 454 sets parsed as 0). **Header keys are single-valued — the first occurrence of each wins** (`takeHeaderKey`), because that terminator is still the only one the header has: when a scrape fails to strip tags at the `Biography :` field the rest of the page lands *ahead* of it, and every thenude.com gallery credit is an anchor whose `title` attribute carries a newline, so lines reading `ICGID: MB-003G" class="model-title">…` match `startsWith('ICGID')`. Under plain assignment the **last** of 93 such lines won: Matilda Bae (MB-003G) staged 111 sets under an ICG-ID with HTML welded to it, every `subjectPersonId` resolved to null, and the cover basket — whose candidate list is `subjectPersonId OR subjectIcgId` — matched nothing. Hand-correcting the real header line could not help, it was overwritten 92 times below. 4 of 164 xpulse files carry the shape; guarding assignment leaves the cursor exactly where it lands today (verified: 163 of 164 files re-parse byte-identical). Repair for rows written before this: `scripts/repair-malformed-subject-icgid.ts`. **`checkParseCoverage(content, parsed)`** counts the block shapes the parser itself keys on straight off the raw text and reports any section the file demonstrably holds that came out empty — a total-failure check only, deliberately silent on partial shortfalls. It **reports, never refuses**: `createBatch` still imports everything that parsed and writes `describeShortfalls()` to `ImportBatch.notes`, surfaced amber in the upload modal (which then drops the "Import complete" heading) and permanently in the `ImportWorkspace` header. `createBatch` returns `CreatedBatch` = batch + `parseShortfalls`. `ParseShortfall` is a discriminated union: `kind: 'section'` is the coverage check above; **`kind: 'malformedIcgId'`** fires when the parsed ICG-ID fails `ICG_ID_RE` — the identity key is the one field whose corruption is silent (the import succeeds, the sets stage, nothing resolves to a person), so it is reported on the same amber surface. Reported, never repaired: the parser cannot know which part of a polluted string was meant.
...
**`staging-set-service.ts`** — StagingSet CRUD + querying. Filters `hasPersonEvidence` (`stagingSetIdsWithPersonEvidence`, 72 rows on xpulse — the suggestion's folder carries a claim or marker for somebody the set credits) and `archiveNamesOthers` are resolved to id lists rather than Prisma filters: both compare two tables against a cast that has two shapes. `getStagingSetsFiltered` (paginated, filterable by status/person/channel/date/priority/search), `getStagingSetStats`, `getStagingSetComparison` (side-by-side diff vs production Set), `updateStagingSetFields`, `bulkUpdateStatus`, `markStagingSetPromoted` (copies `archiveKey` to promoted Set and linked ArchiveFolder), `getDuplicateCandidates(id)` (the staging set(s) that triggered a duplicate warning — confirmed: same `duplicateGroupId`; probable: same channel + release date AND `isDuplicate`, which excludes the split photo/video **sibling**, `StagingSet.siblingId`). Lifecycle statuses: PENDING → REVIEWING → APPROVED → PROMOTED / INACTIVE / SKIPPED. `StagingSetWithRelations` includes optional `suggestedArchiveFolder?: SuggestedFolderInfo | null` (populated at API layer, not in Prisma include).

**`import/cover-transfer.ts`** — `transferStagingCoverToSet(coverImageUrl, setId)`, run at promotion when the Set has no cover of its own. Downloads the staged cover from MinIO, re-generates the standard variants, and creates a MediaItem in the Set's primary session — under `session/{sessionId}/…` and carrying `hash` + `phash`, so it is an ordinary picture of the set rather than a species of its own invisible to `/media/similar`. **`isProvisionalCoverKey(key)`** (pure, unit-tested) decides whether the cover is a stand-in: `archive/…` is a 512 px thumbnail of one of the folder's own images and steps aside for the real upload (`Set.coverIsProvisional`, honoured by the cover assignment in `createMediaItemDirect`); `staging/…` is the publisher's cover art — a different photograph from anything in the set — and keeps the slot. See ADR-0031; backfill for rows predating this: `scripts/backfill-transferred-covers.ts`.
...
- `lib/archive-cast-gap.ts` — **`archivePeopleMissingFromCast(archivePeople, cast)`** (pure, unit-tested): who the archive names that the set does not credit. Counts your confirmed claims **and** hand markers (`FOLDER_ATTRIBUTION`), never catalogue suggestions — 5,074 machine guesses would flag half the browser. Outside the service layer because the staged-sets browser is a client component. Fed by `attachArchiveCastGap` (per page of staged-set rows), `stagingSetIdsWhereArchiveNamesOthers` (the `archiveNamesOthers` filter) and `getSetArchiveCastGap` (the Set page, which also reports whether the person is a curated Person — a promoted cast is `SetParticipant`, a cache of `SessionContribution`, and both require one, so a Contact-only name cannot be added at all).
- `attachFolderPeople(items)` — fills `ArchiveFolderEntry.people` for a page of rows: `claims` (`ArchiveFolderAttribution`) and `cast` (the credit list behind a CONFIRMED link, via `loadCasts`), kept apart per ADR-0028. A post-step over the finished list because the entry is assembled at three call sites and the cast cannot be reached by a join. Two queries per page (≤200 rows).
- `createStagingSetFromOrphan(folderId, overrides?)` — **the only place a folder becomes a staged set.** Writes `releaseDatePrecision: DAY` alongside the date (both sources are day-precise: the folder name carries a full `YYYY-MM-DD`, the dialogue's field is a day). It used to leave the schema default `UNKNOWN`, which propagates to the Set and the Session at promotion and makes `computeProductionAge` return nothing — a folder-born set showed **no participant ages at all** though the date and the birthdate were both stored. Repair for older rows: `scripts/repair-folder-date-precision.ts`. Both entry points route through `createStagingSetFromFolder` (attribution-confirm-service): the develop queue with the folder's attributions and no overrides, the archive row's dialogue with the operator's corrections. It reuses an existing staged set via `findExistingStagingSet` instead of creating a twin, carries the folder's cover, writes `titleNorm`, the CONFIRMED link with `archiveFieldsFromFolder`, `review.develop = DEVELOPED`, and leaves the set `PENDING`. Before this the dialogue built its own StagingSet through `createManualStagingSetAction` and differed on all five (participant unresolved, no cover, no duplicate guard, no review state, `APPROVED`) — that action is now the path for a set with **no** folder.
- `SuggestedFolderInfo.personEvidence` — `{ kind: 'claim' | 'marker'; names }` when the proposed folder carries **your** statement (a confirmed `ArchiveFolderAttribution`, else a `FOLDER_ATTRIBUTION` marker) about somebody the target set credits, matched by **ICG-ID**. Rendered as its own badge beside exact/approx in the staged-set row: the confidence measures structure (day, title), this says the identity is established. Deliberately **not** in `scoreArchiveMatch` — measured on 2,841 live suggestions, 68 carry a claim and 30 a marker and *none* would change tier, so a scoring rule would move nothing today while being free to misfire later. It does act as a **tiebreaker** in `pickBestArchiveCandidate` (`personConfirmed`), ranked above title similarity and below the exact day, and it never turns an unqualified candidate into a suggestion — same person + same channel + same year is true of dozens of pairs.
- `beatsIncumbent(challenger, incumbent)` + `rankArchiveCandidates(...)` (pure, unit-tested) — the matcher streams folder by folder and used to skip any staged set already carrying a link, so the **first** folder to arrive kept it however badly it fitted. Measured on xpulse: 550 of 2,837 suggested links hold a set whose day they do not share, and for **271** a folder that does share the day sat unlinked. A `SUGGESTED` incumbent is now contested, and only a **categorical** improvement wins: the challenger shares the release day and the incumbent does not, or the day is equally (un)matched and the challenger carries a confirmed person. Title similarity never displaces — that is the signal behind "Feel Good"/"Feels Good" five months apart. Equal evidence keeps the incumbent, which makes the pass order-independent. A `CONFIRMED` link is never contested. When the best candidate is unwinnable the next is tried (`firstClaimable`), so a folder does not end empty while a free set sits one line below.
- `archiveFieldsFromFolder(folder)` (pure, unit-tested) — the archive fields a new link inherits: path, file count, video flag, and `OK` (or `MISSING` if the last scan could not find the folder). **Never `PENDING`** — that status means "a path was recorded, nothing has looked at it", which cannot be true of an `ArchiveFolder` row, since a scan created it. Shared by `confirmArchiveFolderLink`, `createStagingSetFromOrphan` and `linkFolderToStagingSet`; the latter two used to write `PENDING`, so a set developed from its own folder displayed a blue badge that reads as an unconfirmed link with no confirm button beside it. Repair for older rows: `scripts/repair-pending-archive-links.ts`.
...
**`archive-cover-service.ts`** — Cover thumbnails for archive folders (plan slice 1). `getCoverWorklist(scope?)` (LIKE-escaped, case-insensitive, path-separator-normalised), `setArchiveFolderCover`, `setArchiveFolderCoverError`, `clearArchiveFolderCover` (delete a cover in the browser so a single folder can be re-read — a cover swap is meant to be the exception, not a `-Refresh` sweep), `listCoverFailures`, `getCoverStats`. Agent: `scripts/archive-cover.ps1`.
...
**`attribution-suggestion-service.ts`** — Stores what the join proposes and folds it into the unit an operator decides on (ADR-0027, plan slice 4). `parseSuggestionBatch` (pure; all-or-nothing validation), `ingestSuggestions` (replaces per `(folder, source)` in a transaction per folder — idempotent re-runs, other sources untouched), `getSuggestionStats`, `aggregateAttributionGroups` (**pure**: `(channel, alias)` groups, one vote per folder per person, demotions carried up, silent groups kept). A group carries `unanimous` — every *suggested* folder names every voted person — because `votes.length > 1` alone cannot separate a disagreement from a multi-person set (`FJ Michelle & Rebecca` is one decision, not a dispute), and 22.2% of catalogue sets name more than one participant + its query wrapper `getAttributionGroups`. Suggestions carry `tier` (`EXACT|STRONG|WEAK`) and `demotions[]` (`CROSS_LABEL|UNKNOWN_CHANNEL|AMBIGUOUS|DATE_VARIANT`) — *why* it is doubtful, not just how much. Nothing here materialises a Set, participant or Contact. Agent: `scripts/catalogue-join.ts --post`.

**`lib/channel-label.ts`** — `channelDisplay(item)` → `{ label, importedAs }` (pure, unit-tested). `StagingSet.channelName` is **provenance**, not the display source, and it disagrees with `Channel.name` in two different ways: **26,340 rows are the same name respelled** ("KATYA CLOVER" vs "KatyaClover") — noise, folded into the record's name; **1,955 rows across 42 channels are a different name mapped on purpose** ("ALS ARCHIVE" → ALSScan, "ONLYTEASE COVERS" → OnlyTease: a base channel and its archive/cover feed) — kept beside the label, because it carries what the record does not *and* because a wrong mapping ("ALSANGELS" → ALSScan) is visible only through it. The test for "merely respelled" is case, accents and separators, deliberately nothing cleverer. `channelLabel` returns just the label. Not used in the import-vs-set comparison grid, where the raw value is the subject of the comparison.

**`lib/media-url.ts`** — `buildUrl` plus **`localMediaUrlOrNull`**, the guard for a standing rule: **the app never renders or fetches media from an external URL.** Harvested URL fields (`Contact.thumbUrl`, some `StagingSet.coverImageUrl`) point at the source site, which is hotlink-protected — 200 to a request with no `Referer`, **403 to a browser**, which always sends one — so they render as broken-image icons and leak a referrer. They are provenance, never a rendering source. `next.config.ts` restricts `images.remotePatterns` to the MinIO host, but that binds `next/image` only; a raw `<img src>` walks past it, so the guard belongs where a URL leaves the service layer. Want a remote image? Import it into MinIO with an agent.

**`person-reference-service.ts`** — `resolvePersonReferences`, a face to compare an archive cover against. Deciding "is this her" from a name is guesswork, and every comparable tool puts a reference image beside the candidate. The difficulty is coverage: of 5,074 suggested identities only 94 are curated Persons and 711 are Contacts, so the resolution runs down a ladder — **Person headshot → catalogue portrait → covers of folders already confirmed as that person**. `Contact.thumbUrl` is deliberately not a rung — it is hotlink-protected and cannot render. The last rung needs no import and no agent, sits at 100% cover coverage, and improves as the operator works.

**`catalogue-avatar-service.ts`** — person portraits harvested from the catalogue, keyed on the **ICG-ID alone** because 4,269 of 5,074 suggested identities have neither a Person nor a Contact to hang one off. Mirrors `archive-cover-service.ts` (`key`/`error`/`checkedAt`): one bad image fails one person, the reason is stored rather than logged, a re-run retries only failures, and Sharp stays strict. Agent: `scripts/catalogue-avatar.ps1`; endpoints `GET /api/catalogue/avatars` (which ICG-IDs are already held, so a re-run uploads only what is new) and `POST /api/catalogue/avatar/[icgId]`.

**`lib/archive-match-label.ts`** — `describeMatchLabel`, pure and shared by the staging browser, the archive banner and the media queue so they cannot drift into three vocabularies. **A badge must never claim agreement it does not have.** The old label was inferred from the confidence alone (`HIGH ? 'date+code' : 'title match'`), but HIGH can be earned by an exact title on its own — so a folder dated 2018-02-10 was announced as a "date+code" match for a set released 2018-02-13. `SuggestedFolderInfo` now carries the measured truth (`dateMatches`, `titleMatches`, `dayDelta`) and the label states the drift in days, so a wrong folder date is visible instead of confirmed on trust.
...
**`services/conflict-session-service.ts`** — `getConflictSession()` builds the session rows (folder +
cover + the contested claim + the target's structured cast + faces for everyone via
`resolvePersonReferences`), and `resolveConflict(folderId, icgId, answer)` applies one of the three
answers. `components/archive/workbench/use-view-prefs.ts` holds the `I`/`T` preferences in a
`useSyncExternalStore` module store, so both sessions share them without a setState-in-effect.
...
**`attribution-confirm-service.ts`** — Per-folder attribution (ADR-0027, plan slices 5–6). `confirmAttributionGroup` writes `ArchiveFolderAttribution` rows and mints ghost `Contact`s (ADR-0022) — **it creates no Set, StagingSet or Person**. That split is measured, not stylistic: only 1.7% of the 28,377 suggestions point at a Person the app already has and 84.8% of suggested identities are unknown entirely, so bulk confirmation is identity *registration*; materialisation stays the curated per-person import path, which now arrives pre-informed about which folders are whose. **The unit of decision is the folder, not the group** (slice 6). `AA | Anna` holds several genuinely different people under one folder alias, so a group-level confirm could attach the wrong person to dozens of sets — the same failure Apple Photos documents when its batch confirm pulls in look-alikes and merges two people into one album, and the reason FamilySearch offers no bulk accept at all. `confirmFolderIdentity` / `rejectFolderIdentity` / `skipFolderIdentity` / `undoFolderIdentity` each act on one folder; `confirmFoldersIdentity` takes a hand-built selection (nothing is ever pre-selected, so the safety property sits in the selection). State lives in `ArchiveFolderReview` (`identity` × `develop`), written in the same transaction as the attribution rows. `planAttributions` remains pure and tested. Also `decideAttributionGroup` (`NOT_A_PERSON` / `SKIPPED` — the largest group in the archive is `W4B | w4b magazine`, 204 folders of a magazine title), `undoAttributionGroup` (drops verdict + its attributions; keeps the deduplicated contacts), `getAttributionQueue` (leverage-ordered; a group leaves the open view when its folders are all ruled on, and group-level `NOT_A_PERSON` / `SKIPPED` survive the agent's next run), `getGroupFolders` (resolves `coverUrl` server-side; `buildUrl` reads the tenant bucket via AsyncLocalStorage). **Stage 2** — `getDevelopQueue` (confirmed folders with no staging set, grouped by *person* because that is what the question is about), `developFolder` — **links to an existing StagingSet rather than duplicating it** (`findExistingStagingSet` / `linkFolderToStagingSet`: exact date + exact normalised title + no archive link; the channel is *reported*, not required, because folder codes and staging channel names legitimately diverge — `W4B` ↔ `WATCH4BEAUTY` — while a real disagreement means the key coincided). Coming at the archive from the disk side, 106 folders on xpulse corresponded to an unlinked staging set that arrived through a person's import. Otherwise it reuses `createStagingSetFromOrphan` + `addStagingSetParticipant` so the ICG-ID reaches promotion. Also `waitOnFolder`.

**`staging-set-participants.ts`** — `addStagingSetParticipant`, extracted from `addStagingSetParticipantAction` so the archive develop path writes participants through the same code. A staging set carries four derived fields (`participants`, `participantStatuses`, `participantIcgIds`, `participantNamesNorm`) that must stay in step; two implementations is how they drift.

**`lib/attribution-candidates.ts`** — `candidatesForFolder`, **pure and outside the service layer on purpose**: the review grid is a client component and the service imports Prisma, so sharing through the service would drag `node:async_hooks` into the browser bundle. Offers the folder's own suggestions first, then the group's votes by weight — a shortcut to a keystroke, never an answer.
...
**`database-maintenance-service.ts`** — Orphan cleanup, duplicate detection, view refresh, `auditChannelDuplicates()` (read-only: names identical once separators and case are ignored — `Channel.nameNorm` keeps spaces and has no unique constraint, so "TeamSkeet" exists twice under codes TMSK/TSKT; plus channels sharing an archive folder, which is legitimate for ATK-ATK's five publications and listed to tell the cases apart), `auditIcgIdOrigins()` (ADR-0026 — read-only: external/self split, IDs matching neither shape, contacts carrying the reserved marker), `auditCatalogueAvatars()` (portrait coverage + the files the agent could not decode), `checkUndefinedArchiveChannels()` (read-only: archive short codes with no Channel behind them, plus channels missing a short code or an owning Label — all three make the join's cross-label guard fail open)
...
| `/archive/conflicts` | The contradiction session (ADR-0028). Same shell as the workbench — `LoupeView`, `WorkbenchFilmstrip`, the shared `useViewPrefs` store — but a different question: which of the two claims is the person on this cover. Three answers per folder: `1` the set is right (`rejectCandidate` — the claim goes, the person joins `rejectedIcgIds`, identity closes only with the last claim), `2` I am right (`addStagingSetParticipant`), `3` wrong link (`unlinkArchiveFolder`, claim intact). On a **promoted Set** `2` is a link to the set instead of a key: `SetParticipant` is a cache rebuilt from `SessionContribution`. The list is derived from `getAttributionLinkAudit()`, so an answered folder simply stops appearing |
| `/archive/workbench?folder=…` | The same workbench about **one folder**, opened from the pencil in the archive list. `getWorkbenchFolderSession` → `getWorkbenchFolder`, **the only loader that does not apply `UNSETTLED_FOLDER`** — a folder whose link was confirmed is reachable nowhere else, and that is exactly the one you open this way (guarded by `folder-people-editing.test.ts`). No group behind it: no votes, no progress bar, no filter tabs, no filmstrip/grid, person-led mode off (it needs a candidate carrying ≥60 % of a group). `Esc` returns to `/archive?highlight=…` |
| `/archive/workbench?group=…&from=…` | The workbench: one group, the whole screen. `WorkbenchClient` → `WorkbenchInspector`, `WorkbenchFilmstrip`, loupe/grid views, `PersonAssignPicker`. Anatomy borrowed from culling software (Narrative Select): large image, filmstrip beneath, inspector beside. Two questions — **person-led** (`J`/`N` about a pinned person) and **folder-led** (`1…9` among candidates), `M` switches, `G` reaches the contact sheet, `Esc` returns to the queue view it came from. The loupe puts the folder name **centred directly above the cover** and the reference face in the frame's top-right corner (Lightroom's Loupe Info pattern), so one fixation covers the whole decision and the eye moves in one axis only; the inset is width-capped to the measured letterbox column so a portrait cover is not occluded, and the title row holds a fixed height per level so the frame never moves between folders. `I` cycles the overlay, `T` hides the filmstrip, both persisted |
| `attribution-actions.ts` | Stage 1 per folder: `confirmFolderAction`, `confirmFoldersAction` (hand-built selection), `rejectFolderAction`, `skipFolderAction`, `undoFolderAction`. Group level keeps only the verdicts that really are about the group: `markGroupNotAPersonAction`, `skipGroupAction`, `undoAttributionGroupAction`. `getGroupFoldersAction` loads members on expand, not with the list. Stage 2: `developFolderAction`, `waitOnFolderAction` |
...
| `/api/archive/rematch` | POST / GET | POST starts the folder→set matching pass (`runMatchingPass`) detached and answers at once; GET returns its progress. Progress lives in `Setting` (`archive-rematch-status.ts`: `startedAt`/`progressAt`/`processed`/`total`/`suggested`/`error`) so it survives the tab that started it, and a heartbeat older than 5 min counts as dead — otherwise one crashed run blocks the button for ever. POST 409s while a run is live. **Not** the refresh on `/staging-sets`: that one matches staged sets against production Sets and never touches the archive; confusing the two made a working matcher look broken. Single-set equivalent: `rematchItemAction` → `runMatchingPassForItem`, offered on the set page and the staged-set panel. |
...
| `/api/archive/cover-worklist` | GET | Protected by `ARCHIVE_API_KEY` + `x-tenant-id` (agent endpoint, plan slice 1). Folders still lacking a thumbnail (`getCoverWorklist`, `archive-cover-service.ts`). Optional `?scope=` path prefix — LIKE-escaped and separator-normalised to match `archive-scan.ps1`'s paths. |
| `/api/archive/cover/[archiveKey]` | POST | Protected by `ARCHIVE_API_KEY` + `x-tenant-id`. Multipart `file` = the downscaled cover the agent read off the archive. Stores `ArchiveFolder.coverKey`; failures land in `coverError` so a bad source is visible rather than silently retried forever. |
| `/api/archive/attribution-worklist` | GET | Protected by `ARCHIVE_API_KEY` + `x-tenant-id` (agent endpoint, ADR-0027). **Read-only.** `{ counts, orphans, groundTruth, channels, aliases }` — unlinked folders, folders already linked to a set with known participants (so the agent can measure recall against known answers), channel→owning-label (ADR-0020, to tell a benign channel disagreement from a cross-label one), and **channel-scoped** aliases (ADR-0024). Optional `?limit`. |
| `/api/archive/attribution-suggestions` | POST | Protected by `ARCHIVE_API_KEY` + `x-tenant-id` (agent endpoint, ADR-0027). `{ source, suggestions[] }` → `ArchiveFolderSuggestion` rows. **Materialises nothing** — confirmation is the only door into person data. Validates the batch whole (a bad entry rejects all of it); replaces per `(folder, source)` so a re-run is idempotent and other sources survive. Unknown `archiveKey`s are skipped and counted. |
...
page.tsx (Server Component — calls ~12 service functions)
  └── PersonDetailTabs (Client — receives all data as props, manages tab state)
        ...
        ├── CareerTab — Unified chronological timeline (ADR-0011). The timeline draws promoted Sets **plus the full in-pipeline staged set** (`buildStagedWhere`: `status IN (PENDING,REVIEWING,APPROVED)`, `matchedSetId: null`, no archive gate) — each row carries a `rowStatus` (`promoted|approved|reviewing|pending`) rendered as a 4-value confidence-graded pill/stripe/tint (`status-pill.tsx`: emerald/cyan/amber/blue). Header stack: compact career row · **Status quick-filter pills** (per-state toggle via `cstatus` URL param, live counts from the `status` facet) · multifacet toolbar (channel/**label**/rating/era/archive — channel & label dropdowns filter to count > 0; label options from `deriveAffiliations(workHistory, stagingWorkHistory)`; URL params `channel`, `clabel`, `crating`, `era`, `archive`, `cstatus`, `csort`) · Sort · **Group** (`year`|`status`|`channelYear`) · **Density** (`full`|`compact`) toggles (group/density/collapse remembered per person in localStorage) · `CareerStatsStrip` (claimed-vs-promoted-vs-in-pipeline-vs-verified gap table: photosets/videos/covers × Claimed/Promoted/In pipeline/Verified/Recorded/Missing; covers derived; data from `getCareerStats()` — claimed read from `Person.claimedPhotosets/claimedVideos`, promoted = `set.count` by type, `staged` = pipeline `stagingSet.count` with `matchedSetId: null` (matches the timeline rows), `stagedVerified` = that subset with a CONFIRMED `archiveLinks`; claimed parsed from the imported biography by `parseClaimedStats()`, guarded against re-import by `Person.claimedStatsUserSet`, with a free-text `Person.claimedStatsNote` provenance shown under the table) · type tabs (with `have/claimed` badges). Timeline driven by `career-service.ts` primitives: `TimelineSection` (year/status/channel→year grouping via `buildSections`, sticky collapsible headers + collapse-all/expand-all, per-row compact→full expand, narrow density-by-bg-fill `YearScrubber` in year mode), `TimelineSetRow` (`variant="full"`: cover · meta lines · status pill · archive pill · star rating; 3rd line for co-participants on multi-cast sets; right-side strip of 4 sample thumbnails for promoted photo rows · `variant="compact"`: single-line date · channel · title + status/archive/rating). Cover-hover `SetHoverPreview` shows just an enlarged cover (240×320 photos / 480×270 videos) anchored to the small cover with viewport-clamped fallback.
...
- **SetCreditRaw**: `resolvedPersonId` / `resolvedAliasId` / `resolvedArtistId` / **`resolvedContactId`**. The last one (plan slice 2) holds a participant whose ICG-ID is known but who has no curated Person yet — without it, promotion dropped the key and the credit decayed to a bare name resolvable only by ambiguous name matching. `repointContactToPerson` moves these onto `resolvedPersonId` (status → RESOLVED) **before** deleting the contact; the FK is `onDelete: SetNull`, so skipping that repoint would silently strip the key rather than fail
