...
**`lib/channel-label.ts`** — `channelLabel(item)`: the `Channel` record's name when the row resolved to one, the import file's spelling only when it did not (11 staged sets). `StagingSet.channelName` is **provenance**, not the display source — it differs from `Channel.name` for 27,684 of 28,295 staged sets ("KATYA CLOVER" vs "KatyaClover"), which made one channel look like two. Same precedence `resolveCreditedAs` applies to credit names (ADR-0024). Deliberately not used in the import-vs-set comparison grid, where the raw value is the subject of the comparison.
