...
| `/archive/workbench?group=…&from=…` | The workbench: one group, the whole screen. `WorkbenchClient` → `WorkbenchInspector`, `WorkbenchFilmstrip`, loupe/grid views, `PersonAssignPicker`. Anatomy borrowed from culling software (Narrative Select): large image, filmstrip beneath, inspector beside. Two questions — **person-led** (`J`/`N` about a pinned person) and **folder-led** (`1…9` among candidates), `M` switches, `G` reaches the contact sheet, `Esc` returns to the queue view it came from. The loupe carries the folder name and the reference face **on the cover** (Lightroom's Loupe Info pattern) so one fixation covers the whole decision — anchored to the frame's corners and width-capped to the measured letterbox column so a portrait cover is not occluded; `I` cycles the overlay, `T` hides the filmstrip, both persisted |
