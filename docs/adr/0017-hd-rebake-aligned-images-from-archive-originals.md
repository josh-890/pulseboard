...
Decided 2026-06-14 (design review, /grill-with-docs — not yet implemented). Builds on ADR-0013 (Aligned images are baked derived copies) and ADR-0014 (templates bind to a MediaCategory).
...
- **D2 — Agent-side, not browser, not server.** The agent reads `{fullPath}\{filename}` off the local disk (the browser can't auto-read local paths even on that machine; the server can't reach them at all), bakes at full resolution with Sharp, and **POSTs back only the small baked result**. The multi-MB original never transits the network or lands in MinIO — consistent with the "don't store originals" ethos.
...
- New: a `MediaItem.bakeSource` (or equivalent flag) for the worklist query; keypoints stored/derived as **fractions** so replay is resolution-independent (legacy pixel-point provenance normalized via the source's master dims at re-bake time); a worklist endpoint + an in-place re-bake endpoint (agent-authenticated); the agent re-bake mode (reuses `lib/image/similarity-transform.ts` + Sharp `affine`). The Sharp-affine result must match the canvas bake — an implementation concern, not a design one.
...
Designed 2026-06-14; **not yet implemented**.
