...
#### Step 1 — First scan (discover folders)
...
No `archiveKey` values exist yet at this point — no link has been confirmed.

---

#### Step 2 — Confirm links in the UI
...
**This is the step that generates the `archiveKey`.** The server creates a UUID and writes it to the `ArchiveFolder`, `StagingSet`/`Set` records at the moment you confirm. The dot turns green immediately.

---

#### Step 3 — Write sidecar files to disk

Once you have confirmed as many links as you want, run a second scan with the `-WriteSidecars` switch:
...
1. **Preload** — the script downloads all `ArchiveFolder` records including their `archiveKey` values. Only folders with a confirmed `archiveKey` are eligible for sidecar writing.
2. **Walk + ingest** — the normal full-scan delta runs first (unchanged folders are skipped).
3. **Write-Sidecars phase** — after ingest, the script iterates all linked folders:
   - If `_pulseboard.json` already exists in the folder → **skipped** (never overwritten).
   - If the folder path doesn't exist on this machine → silently skipped.
   - Otherwise → fetches `GET /api/archive/sidecar/{archiveKey}` from the server and writes the returned JSON as `_pulseboard.json` into the folder.

The output reports:

```
Writing sidecar files (_pulseboard.json)...
  Checking 142 linked folder(s) for missing _pulseboard.json...
  [SIDECAR] D:\Sites\FJ\2023\2023-05-14-FJ Jane Doe - Beach Day\_pulseboard.json
  ...
  Sidecars written: 38 | Already present: 104
```

Use `-DryRun` to preview which folders would receive a sidecar without writing anything:

```powershell
.\archive-scan.ps1 -Mode Full -PhotosetRoot "D:\Sites\" -WriteSidecars -DryRun ...
```
...
- `archiveKey` — the stable UUID (source of truth for move detection).
- `setId` / `stagingSetId` — whichever DB record owns this folder (`null` for the other).
- `title`, `releaseDate`, `channel` — informational only; not used by the scan logic.
...
On every future scan, the script reads `_pulseboard.json` from each folder it visits. If it finds an `archiveKey` but the folder's path is not in the preloaded known-paths list (i.e. the folder looks "new"), the script classifies it as a **MOVE** instead of a **CREATE**:
...
- Propagates the new `archivePath` to the linked `Set` or `StagingSet`.

This works transparently across drive letters (e.g. `D:\` → `E:\`) and across archive root changes. **No re-linking is needed.**
...
|------|---------|---------|
| Initial setup | `-Mode Full` (no `-WriteSidecars`) | Discover all folders, generate suggestions |
| After confirming links in UI | `-Mode Full -WriteSidecars` | Write sidecars into newly confirmed folders |
| Routine (daily/weekly) | `-Mode Full` | Detect new/renamed/moved/deleted folders |
| After bulk archive reorganisation | `-Mode Full -WriteSidecars` | Catch any new confirmations + write missing sidecars |

> **Tip:** `-WriteSidecars` is safe to include on every routine run — it skips folders that already have a sidecar, so it only does work when needed.
