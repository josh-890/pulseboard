...
- **Expanded row view** (Phase G Slice 12) — clicking a body feature row expands it into a 4-section panel: an action toolbar (pin / photos / edit / delete) plus a prominent **Status pill** (● Present / ▲ Modified / ○ Removed / ◐ Overgrown) at the top-right; then labelled **PROPERTIES** (folded current values from the event log — motif / colors / size / description for marks; material / gauge / description for modifications), **PHOTOS** (existing thumbnail strip), and **LIFECYCLE** (dot-line event timeline plus a textual `+ Record event` button). Sections with no content are hidden entirely.
