/**
 * The people files that sit in an archive folder (ADR-0029).
 *
 * Two files, one line format:
 *
 *   `_pulseboard_people.txt` — generated by the app, refreshed by the scan agent
 *   `_people.txt`            — hand-written, read-only to the app
 *
...
export const APP_PEOPLE_FILE = '_pulseboard_people.txt'
export const HAND_PEOPLE_FILE = '_people.txt'
...
export function parsePersonLine(line: string): FilePerson | null {
  ...
  if (withBrackets) {
    const name = withBrackets[1].trim()
 ...
 * hand-written `_people.txt` is, and it carries no markers.
 */
...
export function renderPeopleFile(input: RenderInput): string | null {
  ...
  const lines = [
    `# pulseboard — generated, do not edit. Your own entries belong in ${HAND_PEOPLE_FILE}`,
